...
class LogisticRegressionSGD:
    ...
    def fit(self, X, y):
        ...
        self.alpha=self.alpha/(1-2*self.eta*self.mu)
        # for i in range(n_feature):
        #     self.weight[i] = (1-2*self.mu*self.eta)*self.weight[i]
        for i in range(len(X)):
            self.weight[X[i][0]] = self.weight[X[i][0]]+self.alpha*self.eta*(y - p)*X[i][1]

    def predict(self, X):
        return 1 if self.predict_prob(X) > 0.5 else 0

    def predict_prob(self, X):
        return 1.0 / (1.0 + math.exp(-math.fsum((self.weight[f]*v/self.alpha for f, v in X))))
