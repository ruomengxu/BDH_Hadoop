...
-- ***** your code below *****
FROM (SELECT patient_id, count(*) AS event_count FROM alive_events GROUP BY patient_id) t;
...
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT patient_id, count(*) AS event_count FROM dead_events GROUP BY patient_id) t;
...
-- ***** your code below *****
FROM (SELECT patient_id, COUNT(DISTINCT time) AS encounter_count FROM alive_events GROUP BY patient_id) t;
...
SELECT avg(encounter_count), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT patient_id, COUNT(DISTINCT time) AS encounter_count FROM dead_events GROUP BY patient_id) t;
...
-- ***** your code below *****
FROM (SELECT patient_id, DATEDIFF(MAX(time),MIN(time)) AS record_length FROM alive_events GROUP BY patient_id ) t;
...
SELECT avg(record_length), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT patient_id, DATEDIFF(MAX(time),MIN(time)) AS record_length FROM dead_events GROUP BY patient_id ) t;
...
GROUP BY event_id
SORT BY diag_count DESC LIMIT 5;
...
GROUP BY event_id
SORT BY lab_count DESC LIMIT 5;
...
GROUP BY event_id
SORT BY med_count DESC LIMIT 5;
