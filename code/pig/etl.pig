...
-- ***************************************************************************
eventswithmort = FOREACH (JOIN events BY patientid LEFT OUTER, mortality BY patientid ) GENERATE $0 AS patientid,$1 AS eventid, $2 AS timestamp1,$3 AS value, $5 AS timestamp2, ($6 IS NOT NULL ? $6 : 0)  AS label;
-- perform join of events and mortality by patientid;

deadevents_temp = FILTER eventswithmort BY ($5==1);
deadevents = FOREACH  deadevents_temp GENERATE $0, $1, $3, $5, DaysBetween($4, $2)-30 AS time_difference;
...
aliveevents_temp = FILTER  eventswithmort BY ($5==0);
aliveevents_group = GROUP aliveevents_temp BY patientid;
aliveevents_indx = FOREACH aliveevents_group GENERATE group AS  patientid, FLATTEN(aliveevents_temp.(eventid,value,label,timestamp1)) AS (eventid,value,label,timestamp1), MAX(aliveevents_temp.timestamp1) AS indxdate;
aliveevents = FOREACH  aliveevents_indx GENERATE $0,$1,$2,$3,DaysBetween($5, $4) AS time_difference ;
...
-- ***************************************************************************
filtered = FILTER (UNION aliveevents,deadevents) BY (($4 <=2000) AND ($2 IS NOT NULL) AND ($4>=0));
...
-- ***************************************************************************
feature_group = GROUP filtered BY (patientid, eventid);
featureswithid = FOREACH feature_group GENERATE FLATTEN(group) AS (patientid, eventid), COUNT($1) AS featurevalue;
...
-- ***************************************************************************
get_all_feature = DISTINCT (FOREACH filtered GENERATE $1);
get_all_feature = RANK(ORDER get_all_feature BY eventid); 
all_features = FOREACH get_all_feature GENERATE ($0-1) AS idx, $1;
...
features = FOREACH ( JOIN featureswithid BY eventid, all_features BY eventid) GENERATE $0 AS patientid, $3 AS idx ,$2 AS featurevalue ;
...
-- ***************************************************************************
maxvalues_group = GROUP features BY idx;
maxvalues = FOREACH  maxvalues_group GENERATE group AS idx, MAX(features.featurevalue) AS maxvalues;

-- group events by idx and compute the maximum feature value in each group. I t is of the form (idx, maxvalues)

normalized = FOREACH  (JOIN features BY idx, maxvalues BY idx) GENERATE $0 AS patientid, $1 AS idx, $2 AS featurevalue,$4 AS maxvalues;

-- join features and maxvalues by idx

features = FOREACH normalized GENERATE $0, $1, (double) $2/$3 AS normalizedfeaturevalue;
...
labels = DISTINCT (FOREACH (UNION deadevents, aliveevents) GENERATE  $0,$3);
